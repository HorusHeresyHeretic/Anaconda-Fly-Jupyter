...
В иделае это 2 монитора: в первом студия и разработка, во втором Юпитер с уже полученными результатами в качетве подсказки. Плюс - коллегами будет проще читать ваш код в формате IPython где он разбит по операциям, чем искать глазами нужную строчку в длиннокоде. 
