...
Лучше потратить и то и другое на "подумать" над кодом - чем ломать голову в попытках соотнести то, что вернул интрепретатор сейчас с тем, что он вернул 400 строк вывода назад. Поэтому, я считаю правильным разглядывать результаты вывода на полях Юпитера, так проще.
