...
Смысл в том, что код написаный в студии в рамках задачи по датамайнингу - это длиннокод. И чем шире задача, тем сложнее его читать (см. https://github.com/HorusHeresyHeretic/Learning-Python-from-zero/blob/master/Lesson-8/MachineLearning1.py) - ревизия содержимого датафрейма при попытке "понять" основные индикаторы это порядка 200+ строк кода, который особо не откомментрируешь.
